...
const ManualPRTable: React.FC = () => {
    ...
    const [dateTo, setDateTo] = useState("2029-02-02");
    ...
    const filteredRows = pendingPRData.filter((row) => {
        ...
        // Date range filter (only apply if both dateFrom and dateTo are set)
        if (dateFrom && dateTo) {
    ...
    const renderCellContent = (
        ...
        if (col.key === 'prStatus' && row.prStatus === 'Approved') {
            return (
                <div className="outstanding-pr-value status-with-action">
                    <span>{cellValue}</span>
                    <Link to={`/new-po`} className="pr-link">
                        Create PO
                    </Link>
                </div>
            );
        ...
        <div className="outstanding-pr-root">
            ...
            <main className="outstanding-pr-content">
                <header className="outstanding-pr-header">
                    <h1 className="outstanding-pr-title">Manual PR with RFP</h1>
                    <p className="outstanding-pr-subtitle">
                        View and search manual purchase requests with RFP. Use the filters
